 ...
 * ===================================
 * This plugin helps working with tables with a lot of colums. Clicking the header of a column while holding the CTRL-key hides the column.
	...
	public function head()
	{
		...
		function headerSpanClick(e) {
			...
			if (event.target.children[1] == undefined) {
				...
				column = event.target.children[1].innerText
			}

			if (event.ctrlKey && column !== false) {
				window.location.href = '?<?php echo $_SERVER["QUERY_STRING"]?>&hide[]=' + column;
			}
		}
		</script>
		<?php
